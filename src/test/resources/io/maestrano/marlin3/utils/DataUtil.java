...
//1,0 1,1
	...
	public static boolean isTestExecutable(Xls_Reader xls, String testCaseName) {
		...
		for (int rNum = 2; rNum <= rows; rNum++) {
			String tcid = xls.getCellData(Constants.TESTCASE_XLS_SHEET, "TCID", rNum);
			if (tcid.equals(testCaseName)) {
				String runmode = xls.getCellData(Constants.TESTCASE_XLS_SHEET, "Runmode", rNum);
				if (runmode.equals("Y"))
					return true;
				else
					return false;
